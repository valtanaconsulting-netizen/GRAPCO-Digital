...
> ⚠️ **Hallazgo (jun-2026)**: en `PlanMaestro/creditex-ptar` la data es **demo/desincronizada** (actividades genéricas tipo "porcelanato", "ventanas de aluminio", "caudalímetros" que NO son del PTAR+NAVE real), con la capa de ejecución casi vacía (estado/predecesoras/apuId/petId = 0; solo 49 con fechas). La obra **real** de CREDITEX vive en `Catalogo_WBS`. → Pendiente decidir: limpiar la data demo de `PlanMaestro/creditex-ptar` o repuntar las vistas del módulo Plan Maestro al catálogo. (No se tocó data en esta consolidación.)
