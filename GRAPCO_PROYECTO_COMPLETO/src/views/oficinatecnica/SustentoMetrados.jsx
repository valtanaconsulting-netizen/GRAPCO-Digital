...
export default function SustentoMetrados({ showToast }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      ...
      {/* Modal crear/editar */}
      {modalAbierto && (
        <Modal onClose={cerrar} title={editando ? 'Editar sustento' : 'Nuevo sustento de metrado'} maxW="640px">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            ...
            <Campo label="Metrado">
              <input type="number" step="0.01" value={form.metrado} onChange={(e) => setForm({ ...form, metrado: e.target.value })} style={inp()} />
            </Campo>
            <Campo label="Unidad">
              <select value={form.unidad} onChange={(e) => setForm({ ...form, unidad: e.target.value })} style={inp()}>
                {['m3','m2','m','kg','und','glb','día','h-h'].map(u => <option key={u} value={u}>{u}</option>)}
              </select>
            </Campo>
          </div>
        ...
        <Modal onClose={() => setVerDetalle(null)} title={`${verDetalle.partida} · ${verDetalle.periodoMes}`} maxW="900px">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
              ...
              <Info label="Ubicación" value={verDetalle.ubicacion || '—'} />
            </div>
            <div>
